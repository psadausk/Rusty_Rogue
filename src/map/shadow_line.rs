...
impl ShadowLine {
    ...
    pub fn add(&mut self, shadow: Shadow) {
        let mut index = 0;
        for cur_shadow in &self.shadows {
            if cur_shadow.start >= shadow.start {
                break;
            }
            index = index + 1;
        }

        let overlapping_prev = {
        ...
        match overlapping_next {
            Some(next) => {
                match overlapping_prev {
                    Some(_prev) => {
                        //prev.end = next.end;
